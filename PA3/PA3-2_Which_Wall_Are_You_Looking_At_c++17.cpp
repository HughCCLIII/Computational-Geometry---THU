...
using VData = variant<PointData, SegmentData, TrapezoidData>;


struct SearchNode
{
	SNPPtr lc, rc;
	VData data;					//use std::variant to avoid pointer-casting
	SearchNode(VData&& _data, SNPPtr _lc = SNPPtr(), SNPPtr _rc = SNPPtr()) :data(move(_data)), lc(_lc), rc(_rc)
	{
...
{
	PointNode(const Point& _p, SNPPtr _lc = SNPPtr(), SNPPtr _rc = SNPPtr()) :SearchNode(VData(PointData(_p)), _lc, _rc)
	{
	...
	{
		auto& pData = get<PointData>(data);
		return tp < pData.p || tp == pData.p;
	}

	bool visitLeft(const Segment& ts) const override
	{
		return ts.left < get<PointData>(data).p;
	}
};

struct SegmentNode :public SearchNode
{
	SegmentNode(const Segment& _s, SNPPtr _lc = SNPPtr(), SNPPtr _rc = SNPPtr()) :SearchNode(VData(SegmentData(_s)), _lc, _rc)
	{
...
struct TrapezoidNode :public SearchNode
{
	TrapezoidNode(const Point& _left, const Point& _right, const Segment& _top, const Segment& _bottom, SNPPtr _lc = SNPPtr(), SNPPtr _rc = SNPPtr()) :
		SearchNode(VData(TrapezoidData(_left, _right, _top, _bottom)), _lc, _rc)
	{
...
struct TrapezoidalMap
{
	...
	{
		get<TrapezoidData>(left->get()->data).lowerRight = right;
		get<TrapezoidData>(right->get()->data).lowerLeft = left;
	}

	void setTopNeighbour(SNPPtr& left, SNPPtr& right)
	{
		get<TrapezoidData>(left->get()->data).upperRight = right;
		get<TrapezoidData>(right->get()->data).upperLeft = left;
	}
	...
	void insertSegment(const Segment& s)
	{
		...
		TrapezoidData* curTrapezoid = &get<TrapezoidData>(curTreeNode->get()->data);
		...
		if (s.right > curTrapezoid->right)	//超过一个
		{
			...
			curTrapezoid = &get<TrapezoidData>(curTreeNode->get()->data);
			...
			while (s.right > curTrapezoid->right)
			{
				if (s.toLeft(curTrapezoid->left) > 0)	//梯形左端点在插入线段上方，那么下面的梯形相连
				{
					...
					get<TrapezoidData>(lastBottom->get()->data).right = curTrapezoid->right;
				...
				else
				{
					...
					get<TrapezoidData>(lastTop->get()->data).right = curTrapezoid->right;
			...
			if (s.toLeft(curTrapezoid->left) > 0)
			{
				...
				get<TrapezoidData>(lastBottom->get()->data).right = s.right;
				leftBorderNeighbourhoodCheck(*curTrapezoid, upperTrapezoid, upperTrapezoid);
				setBottomNeighbour(lastTop, upperTrapezoid);
				lastTop = upperTrapezoid;
			}
			else
			{
				...
				get<TrapezoidData>(lastTop->get()->data).right = s.right;
				leftBorderNeighbourhoodCheck(*curTrapezoid, lowerTrapezoid, lowerTrapezoid);
				setTopNeighbour(lastBottom, lowerTrapezoid);
				lastBottom = lowerTrapezoid;
			}
...
int main()
{
	...
	{
		cin >> p1.x >> p1.y;
		TrapezoidData& res = get<TrapezoidData>(TM.query(p1)->get()->data);
		if (res.right.x == p1.x)
		{
			id = res.right.id;
		}
